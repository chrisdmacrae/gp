...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
	<div class="combo-grup table-responsive">
		<table class="woocommerce-grouped-product-list table group_table">
			<thead>
				<tr>
					<th class="group-product-name"><?php esc_html_e( 'Product', 'actavista' ); ?></th>
					<th class="group-product-price"><?php esc_html_e( 'Price', 'actavista' ); ?></th>
					<th class="group-product-quantity"><?php esc_html_e( 'Quantity', 'actavista' ); ?></th>
				</tr>
			</thead>
			<tbody>
			<?php
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					switch ( $column_id ) {
						case 'label':
							...
							break;
						case 'price':
							$value = '<div class="span">' . $grouped_product_child->get_price_html() . wc_get_stock_html( $grouped_product_child ) . '</div>';
							break;
						case 'quantity':
							ob_start();
							if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
								...
								echo '<input type="checkbox" name="' . esc_attr( 'quantity[' . $grouped_product_child->get_id() . ']' ) . '" value="1" class="qty form-control wc-grouped-product-add-to-cart-checkbox" />';
							} else {
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="single_add_to_cart_button shopnow"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
